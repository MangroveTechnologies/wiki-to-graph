...
| **`domain.json`** | JSON (KST) | Optional **Knowledge Space Theory** projection (`items` = concepts, `prerequisites` = the heuristic `--dag-edges` subset). Regenerated by `--kspace`. The prerequisites are candidate suggestions from body references — curate before treating them as a true KST surmise relation. |
